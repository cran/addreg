summary.addreg <- function(object, correlation = FALSE, ...) {
  ...
  switch(object$family$family, poisson = {dispersion <- 1}, binomial = {dispersion <- 1})
  ...
  if(!object$boundary) {
    ...
    if(!inherits(covmat.unscaled,"try-error") | all(is.nan(covmat.unscaled))) {
      ...
      warning("summary.addreg: expected information matrix is singular, could not calculate covariance matrix", call. = FALSE)
